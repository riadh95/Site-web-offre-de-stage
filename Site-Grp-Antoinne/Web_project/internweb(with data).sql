...
CREATE TABLE `delegueoptedintern` (
  `id` bigint(20) NOT NULL,
  `del_id` bigint(20) NOT NULL,
  `intern_id` bigint(20) NOT NULL
  `applied_on` date NOT NULL
...
INSERT INTO `delegueoptedintern` (`id`, `stu_id`, `intern_id`, `applied_on`) VALUES
(1, 2, 2, '2018-05-22'),
(6, 3, 2, '2018-05-23'),
(7, 3, 3, '2018-05-24'),
(8, 7, 4, '2018-05-24'),
...
-- (See below for the actual view)
--
CREATE TABLE `delegue_intern_view` (
`intern_id` bigint(20)
,`del_id` bigint(20)
,`emp_id` bigint(20)
,`organization` varchar(50)
...
-- Structure for view `student_intern_view`
--
DROP TABLE IF EXISTS `delegue_intern_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `delegue_intern_view`  AS  select `delegueoptedintern`.`intern_id` AS `intern_id`,`delegueoptedintern`.`del_id` AS `del_id`,`employee`.`emp_id` AS `emp_id`,`employee`.`organization` AS `organization`,`internlist`.`intern_topic` AS `intern_title`,`internlist`.`intern_location` AS `location`,`internlist`.`intern_duration` AS `duration`,`internlist`.`stipend` AS `stipend`,`delegueoptedintern`.`applied_on` AS `applied_on` from ((`delegueoptedintern` join `internlist` on((`delegueoptedintern`.`intern_id` = `internlist`.`intern_id`))) join `employee` on((`internlist`.`emp_id` = `employee`.`emp_id`))) ;
...
-- Indexes for table `studentoptedintern`
--
ALTER TABLE `delegueoptedintern`
  ADD PRIMARY KEY (`id`),
  ADD KEY `intern_id` (`intern_id`),
...
--
ALTER TABLE `delegue`
  MODIFY `del_id` bigint(20) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8;
...
--
ALTER TABLE `delegueoptedintern`
  ADD CONSTRAINT `delegueoptedintern_ibfk_1` FOREIGN KEY (`intern_id`) REFERENCES `internlist` (`intern_id`),
  ADD CONSTRAINT `delegueoptedintern_ibfk_2` FOREIGN KEY (`del_id`) REFERENCES `delegue` (`del_id`);
COMMIT;
